...
+ a list of search results will come up; when you find the one
  called Debian, by The Debian Project, click on "Get"
...
+ don't forget to write those credentials down and keep them available
  for reference
+ the installer may or may not ask you to reboot your machine,
  depending on your exact setup

Once you've completed the above steps, you should be able to open a
Debian WSL Terminal by running an application called "Debian" from
your Start Menu.  It may or may not ask you to log in; if it does, use
...
it connecting to some websites and downloading some information.
...
You can then navigate back to the source code directory like this:
